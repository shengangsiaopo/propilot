...
int strnicmp(const TCHAR *, const TCHAR *, size_t);
#endif
#endif
...
static void ini_tempnameB(TCHAR *dest, const TCHAR *source, int maxlength)
{
  TCHAR *p;
...
NORMAL_CLEANUP:
  ini_close(&rfp);
  ini_close(&wfp);
  if ( strlen( Filename ) > ((INI_BUFFERSIZE/2)-1) )
  { // old slow method of deleting file - causes mass disk fragmentation
    ini_remove(Filename);
    ini_tempname(LocalBuffer, Filename, INI_BUFFERSIZE);
    ini_rename(LocalBuffer, Filename);
  } else {
//    ini_remove(Filename);
    ini_tempname(LocalBuffer, Filename, INI_BUFFERSIZE);
    ini_tempnameB(&LocalBuffer[INI_BUFFERSIZE/2], Filename, INI_BUFFERSIZE/2);
//    ini_rename(LocalBuffer, Filename);
    ini_remove(&LocalBuffer[INI_BUFFERSIZE/2]);			/* just in case it exists */
    ini_rename(Filename,&LocalBuffer[INI_BUFFERSIZE/2]);
    ini_rename(LocalBuffer, Filename);
    ini_rename(&LocalBuffer[INI_BUFFERSIZE/2],LocalBuffer);
  }
  return 1;
}
...
{
  TCHAR buff[64];
//  _stprintf(buff, __T("%f"), (double)Value);
  return ini_puts(Section, Key, buff, Filename);
}
#endif /* INI_FLOAT */
#endif /* !INI_READONLY */
