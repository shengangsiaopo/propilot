...
//#if sizeof(mavlink_waypoint_set_t) != 29 // sadly this fails 
 ...
 */
static inline uint16_t mavlink_msg_waypoint_set_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint8_t target_system, uint8_t target_component, uint16_t wp_id, uint8_t type, float param1, float param2, uint8 orbit, uint8_t current, float x, float y, float z, uint16_t autocontinue)
{
	...
	return mavlink_finalize_message(msg, system_id, component_id, sizeof(mavlink_waypoint_set_t));
}
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_waypoint_set_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint8_t target_system, uint8_t target_component, uint16_t wp_id, uint8_t type, float param1, float param2, uint8 orbit, uint8_t current, float x, float y, float z, uint16_t autocontinue)
{
	mavlink_waypoint_set_t *p = (mavlink_waypoint_set_t *)&msg->payload[0];
	msg->msgid = MAVLINK_MSG_ID_WAYPOINT_SET;
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, sizeof(mavlink_waypoint_set_t));
}
...
static inline void mavlink_msg_waypoint_set_send(mavlink_channel_t chan, uint8_t target_system, uint8_t target_component, uint16_t wp_id, uint8_t type, float param1, float param2, uint8 orbit, uint8_t current, float x, float y, float z, uint16_t autocontinue)
{
	...
	msg.msgid = MAVLINK_MSG_ID_WAYPOINT_SET;
	msg.len = sizeof(mavlink_waypoint_set_t);
	p->target_system = target_system; // uint8_t:System ID
	p->target_component = target_component; // uint8_t:Component ID
	p->wp_id = wp_id; // uint16_t:Waypoint ID
	p->type = type; // uint8_t:0: global (GPS), 1: local, 2: global orbit, 3: local orbit
