...
//#if sizeof(mavlink_gps_raw_t) != 29 // sadly this fails 
 ...
 */
static inline uint16_t mavlink_msg_gps_raw_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg, uint8_t fix_type, float lat, float lon, float alt, float eph, float epv, float v, float hdg)
{
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_gps_raw_pack_chan(uint8_t system_id, uint8_t component_id, uint8_t chan, mavlink_message_t* msg, uint8_t fix_type, float lat, float lon, float alt, float eph, float epv, float v, float hdg)
{
	mavlink_gps_raw_t *p = (mavlink_gps_raw_t *)&msg->payload[0];
	msg->msgid = MAVLINK_MSG_ID_GPS_RAW;
...
static inline void mavlink_msg_gps_raw_send(mavlink_channel_t chan, uint8_t fix_type, float lat, float lon, float alt, float eph, float epv, float v, float hdg)
{
