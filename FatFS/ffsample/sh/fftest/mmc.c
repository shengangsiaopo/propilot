...
#define _USE_WP		0	/* Use write protect detect switch */
...
static volatile
BYTE Timer1, Timer2;	/* 100Hz decrement timer stopped at zero (disk_timerproc()) */
...
)
{
	...
	RSPI0.SPDR.BYTE;				/* Discard received data */
}
...
BYTE rcvr_spi (void)
{
	...
	while (!RSPI0.SPSR.BIT.SPRF) ;	/* Wait for end of transfer */
	return RSPI0.SPDR.BYTE;			/* Returen received byte */
}
...
static
int wait_ready (void)	/* 1:Ready, 0:Timeout */
{
	Timer2 = 50;	/* Timeout = 500ms */
...
{
	CPG.STBCR5.BIT.MSTP51 = 0;	/* Enable SPI0 module */
...
void power_off (void)	/* Disable SPI function */
{
	...
	CPG.STBCR5.BIT.MSTP51 = 1;	/* Shutdown SPI0 module */
}
...
)
{
	...
	if (!wait_ready()) return 0;		/* Wait for card ready */
