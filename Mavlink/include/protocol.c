 ...
 */
STATIC_FUN void mavlink_parse_state_initialize(mavlink_status_t* initStatus)
{
...
STATIC_FUN inline mavlink_status_t* mavlink_get_channel_status(uint8_t chan)
{
 ...
 */
STATIC_FUN inline uint16_t mavlink_finalize_message(mavlink_message_t* msg, uint8_t system_id, uint8_t component_id, uint16_t length)
{
 ...
 * @param length Message length, usually just the counter incremented while packing the message
 */
STATIC_FUN inline uint16_t mavlink_finalize_message_chan(mavlink_message_t* msg, uint8_t system_id, uint8_t component_id, uint8_t chan, uint16_t length)
{
	// This code part is the same for all messages;
	uint16_t checksum;
	msg->len = length;
 ...
 */
STATIC_FUN inline uint16_t mavlink_msg_to_send_buffer(uint8_t* buffer, const mavlink_message_t* msg)
{
 ...
 */
STATIC_FUN inline uint16_t mavlink_msg_get_send_buffer_length(const mavlink_message_t* msg)
{
	return msg->len + MAVLINK_NUM_NON_PAYLOAD_BYTES;
}
...
STATIC_FUN inline void mavlink_start_checksum(mavlink_message_t* msg)
{
	union checksum_ ck;
	crc_init(&(ck.s));
	msg->ck_a = ck.c[0];
...
STATIC_FUN inline void mavlink_update_checksum(mavlink_message_t* msg, uint8_t c)
{
	union checksum_ ck;
	ck.c[0] = msg->ck_a;
	ck.c[1] = msg->ck_b;
 ...
 * @endcode
 */
STATIC_FUN inline uint8_t mavlink_parse_char(uint8_t chan, uint8_t c, mavlink_message_t* r_message, mavlink_status_t* r_mavlink_status)
{
#if (defined linux) | (defined __linux) | (defined  __MACH__) | (defined _WIN32)
...
/*
STATIC_FUN inline uint8_t mavlink_parse_char_new(uint8_t chan, uint8_t c, mavlink_message_t* r_message, mavlink_status_t* r_mavlink_status)
{
	#if (defined linux) | (defined __linux) | (defined  __MACH__) | (defined _WIN32)
	    static mavlink_status_t m_mavlink_status[MAVLINK_COMM_NB_HIGH];
