...
#define SetCCR1(x) ((void(*)(DWORD))((DWORD)SetCCR1_proc + 0x20000000))(x)	/* Write CCR1 (Executed in non-cacheable area) */
...
void IoInit (void)
{
	...
	CPG.STBCR7.BIT.MSTP72 = 0;		/* Enable CMT */
...
)
{
	...
	if (disk_ioctl(drv, GET_SECTOR_SIZE, &w) == RES_OK)
...
int main (void)
{
	...
	xfunc_in = scif2_get;	/* Join console and SCIF2 */
	xfunc_out = scif2_put;
